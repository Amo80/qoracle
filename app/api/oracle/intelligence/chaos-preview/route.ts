...
import { isChaosIntelligencePreviewIntegrationEnabled } from "../../../../../lib/experience/featureFlags";
...
import { CHAOS_PREVIEW_PROVIDER_TIMEOUT_MS } from "../../../../../lib/oracle-intelligence/timeout";
...
export async function POST(request: NextRequest) {
  if (!isChaosIntelligencePreviewIntegrationEnabled()) return notFound();
  ...
  if (!body || typeof body !== "object" || (body as { oracleId?: unknown }).oracleId !== "chaos") return invalidRequest();

  invocationCount += 1;
  const diagnostics = new PreviewIntelligenceDiagnosticRecorder(instanceId, invocationCount, "chaos-integration", CHAOS_PREVIEW_PROVIDER_TIMEOUT_MS);
  ...
  const result = await runOracleIntelligenceService({ candidateRequest: body, sessionId, provider: createOpenAICompactProviderFromEnvironment(), rateLimiter, signal: request.signal, chaosPreviewMode: true, diagnostics });
  if (!result) return invalidRequest();
  diagnostics.finish();
  const response = NextResponse.json({ ...result, diagnostic: diagnostics.snapshot() }, { status: 200, headers: { "Cache-Control": "no-store", "Server-Timing": diagnostics.serverTimingHeader(), "X-Oracle-Diagnostic": "preview-chaos-integration-v1" } });
