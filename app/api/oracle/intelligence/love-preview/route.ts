...
import { isLoveIntelligencePreviewIntegrationEnabled } from "../../../../../lib/experience/featureFlags";
import {
...
import { LOVE_PREVIEW_PROVIDER_TIMEOUT_MS } from "../../../../../lib/oracle-intelligence/timeout";
...
export async function POST(request: NextRequest) {
  if (!isLoveIntelligencePreviewIntegrationEnabled()) return notFound();
  ...
  if (!body || typeof body !== "object" || (body as { oracleId?: unknown }).oracleId !== "love") {
    return invalidRequest();
  }
  ...
  const response = NextResponse.json(
    { ...result, diagnostic: diagnostics.snapshot() },
    {
      ...
      headers: {
        ...
        "X-Oracle-Diagnostic": "preview-love-integration-v1",
      },
    }
  );
  if (!existingSession) {
