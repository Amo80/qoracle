...
import { JESTER_PREVIEW_PROVIDER_TIMEOUT_MS } from "../../../../../lib/oracle-intelligence/timeout";
...
export async function POST(request: NextRequest) {
  if (!isJesterIntelligencePreviewIntegrationEnabled()) return notFound();
  ...
  const response = NextResponse.json(
    { ...result, diagnostic: diagnostics.snapshot() },
    {
      ...
      headers: {
        ...
        "X-Oracle-Diagnostic": "preview-jester-integration-v1",
      },
    }
  );
  if (!existingSession) {
