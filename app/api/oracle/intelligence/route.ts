...
import { createOpenAIProviderFromEnvironment } from "../../../../lib/oracle-intelligence/openaiProvider";
...
export async function POST(request: NextRequest) {
  if (!isAllowedIntelligenceOrigin(request) || !isJsonContentType(request)) {
    return invalidRequest();
  }
  const declaredLength = Number(request.headers.get("content-length") ?? "0");
  if (Number.isFinite(declaredLength) && declaredLength > ORACLE_INTELLIGENCE_MAX_BODY_BYTES) {
    return invalidRequest();
  }
  let text: string;
  try {
  ...
  if (new TextEncoder().encode(text).byteLength > ORACLE_INTELLIGENCE_MAX_BODY_BYTES) {
    return invalidRequest();
  }
  let body: unknown;
  try {
  ...
  const result = await runOracleIntelligenceService({
    ...
    sessionId,
    provider: createOpenAIProviderFromEnvironment(),
    rateLimiter,
