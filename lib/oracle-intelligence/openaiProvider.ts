...
import type { ProviderDiagnosticRecorder, QualificationReasoningEffort, QualificationReasoningProfile } from "./diagnostics";
...
export function createOpenAIProviderForQualification(
  ...
  profile: QualificationReasoningProfile
) {
  const apiKey = environment.OPENAI_API_KEY?.trim();
  if (!apiKey) return null;
  const client = new OpenAI({ apiKey, maxRetries: 0 });
  const effort = profile === "default" ? undefined : profile;
